 ...
 * @FilePath: /baiying/application/admin/controller/finance/ContractingSubOrder.php
 ...
 * @icon fa fa-circle-o
 */
class ContractingSubOrder extends Backend
{

    /**
     * ContractingSubOrder模型对象
     * @var \app\admin\model\finance\ContractingSubOrder
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\finance\ContractingSubOrder;
    ...
    public function index($ids = null)
    {
        ...
        $query = $this->model
            ->where($where)
            ->alias('so1')
            ->where(['so1.status' => $ids, 'so1.settlement_relationship' => Constants::RELATIONSHIP_P_C])
            ->join('sub_order so2', 'so1.order_id = so2.order_id AND so2.settlement_relationship = 2')
        ...
        so2.to_name as business_company_name,so2.settlement_type as settlement_type2');
    ...
    public function invoice($ids = '')
    {
        ...
        $params['invoice_relationship'] = Constants::RELATIONSHIP_P_C;
    ...
    public function settlement($ids = '')
    {
        ...
        $params['amount'] = round($amount, 2);
        $params['settlement_relationship'] = Constants::RELATIONSHIP_P_C;
